...
# ── Shared state ──────────────────────────────────────────────────────
_bands: dict[str, float] = {"low": 0.0, "mid": 0.0, "high": 0.0}
...
def _audio_callback(indata: np.ndarray, frames: int, time_info, status) -> None:
    ...
    if max(bands.values()) >= 0.15:
        pretty = " | ".join(f"{k.upper()}={v:.2f}" for k, v in bands.items() if v >= 0.15)
        print(f"[Audio] Levels: {pretty}", flush=True)
